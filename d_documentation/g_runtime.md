...
A camada `k_runtime` foi consolidada para gerenciar rigorosamente o ciclo real de vida (execução em disco) do projeto *materializado*. Este runtime encapsulado provê execução segura de subprocessos (`shell=False`).

## Execution
- **a_execution/a_runtime.py**: O motor local que intercepta as tarefas geradas pela fábrica na fase de materialização. Ele se apropria dos caminhos compilados via MCP e dispara testes ou passos de verificação contidos nos `run_commands` ditados pelo Planner.
