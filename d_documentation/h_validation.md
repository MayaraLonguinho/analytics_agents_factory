...
A camada `l_validation` instaura o veredito primário se os processos executados pela máquina `k_runtime` não quebraram durante o disparo inicial.
...
Se o `ValidationGate` travar o progresso reprovando a fase, a estrutura orquestradora (em `o_orchestration`) suspende temporariamente o avanço à Qualidade e aciona a `ProjectPhase.REPAIR_LOOP`.
