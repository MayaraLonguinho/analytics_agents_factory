...
def test_runtime_failure_blocks_readiness():
    ...
    with patch("a_platform.f_llm_gateway.gateway.LLMGateway.generate", side_effect=mock_func), \
         patch("a_platform.i_runtime.runtime_engine.RuntimeEngine.run_project", return_value=False):
...
def test_validation_failure_blocks_readiness():
    ...
    with patch("a_platform.f_llm_gateway.gateway.LLMGateway.generate", side_effect=mock_func), \
         patch("a_platform.j_validation.validation_gate.ValidationGate.run_validation", return_value=False):
...
def test_certification_failure_blocks_readiness():
    ...
    with patch("a_platform.f_llm_gateway.gateway.LLMGateway.generate", side_effect=mock_func), \
         patch("a_platform.l_certification.certification_engine.CertificationEngine.run_certification", return_value=False):
