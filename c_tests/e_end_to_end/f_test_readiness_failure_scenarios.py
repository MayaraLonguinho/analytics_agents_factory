...
from c_tests.e_end_to_end.b_mock_gateway import get_mocked_generate_function
...
def test_runtime_failure_blocks_readiness():
    ...
    with patch("a_platform.g_llm_gateway.gateway.LLMGateway.generate", side_effect=mock_func), \
         patch("a_platform.j_runtime.a_execution.c_runtime.ProjectRuntime.execute", return_value=ExecutionResult(status="FAILED")):
...
def test_validation_failure_blocks_readiness():
    ...
    with patch("a_platform.g_llm_gateway.gateway.LLMGateway.generate", side_effect=mock_func), \
         patch("a_platform.k_validation.a_validation_gate.ValidationGate.run_validation", return_value=False):
...
def test_certification_failure_blocks_readiness():
    ...
    with patch("a_platform.g_llm_gateway.gateway.LLMGateway.generate", side_effect=mock_func), \
         patch("a_platform.m_certification.a_certification_engine.CertificationEngine.run_certification", return_value=False):
