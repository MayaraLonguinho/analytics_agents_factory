...
class MCPExecutor:
    ...
    def _execute_docker(self, definition: MCPDefinition, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Basic policy allowlist check
        allowed_subcommands = {"info", "ps", "images", "build", "run", "stop", "rm", "rmi", "compose", "logs", "inspect"}
        if len(args) > 1 and args[1] not in allowed_subcommands:
            return {"status": "error", "message": f"Subcomando docker '{args[1]}' não é permitido pela policy."}
