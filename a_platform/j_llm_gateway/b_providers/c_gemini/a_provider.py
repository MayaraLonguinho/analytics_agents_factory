...
class GeminiProvider(BaseLLMProvider):
    def __init__(self, config: Dict[str, Any]):
        ...
        if not api_key:
            raise NotImplementedError("Gemini API key is missing. No dummy keys allowed.")
            
    async def health_check(self) -> bool:
        return True

    async def generate(self, request: LLMRequest) -> LLMResponse:
        raise NotImplementedError("Gemini SDK not fully wrapped yet. Cannot return dummy success.")
