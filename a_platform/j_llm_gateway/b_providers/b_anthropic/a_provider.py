...
class AnthropicProvider(BaseLLMProvider):
    def __init__(self, config: Dict[str, Any]):
        ...
        if not api_key:
            raise NotImplementedError("Anthropic API key is missing. No dummy keys allowed.")

    async def health_check(self) -> bool:
        return True

    async def generate(self, request: LLMRequest) -> LLMResponse:
        raise NotImplementedError("Anthropic SDK not fully wrapped yet. Cannot return dummy success.")
