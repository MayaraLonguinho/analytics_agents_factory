...
class PlannerAgent:
    ...
    def generate_plan(self, request: ProjectRequest) -> bool:
        ...
        system_prompt = (
            ...
            "- **Data Layer:** Ingestão e pipeline ETL (agent: DataAgent, skills: [etl_scripting]).\n"
            "- **Database Layer:** Schemas DDL, tabelas, relacionamentos (agent: DatabaseAgent, skills: [sql_generation]).\n"
            "- **Analytics Layer:** Queries analíticas, cálculo de métricas/KPIs (agent: AnalyticsAgent, skills: [sql_generation, basic_coding]).\n"
            "- **Backend Layer:** API REST com rotas, schemas e serviços (agent: BackendAgent, skills: [api_design, basic_coding]).\n"
            "- **Frontend Layer:** Componentes UI/React, Dashboard interativo (agent: FrontendAgent, skills: [basic_coding]).\n"
            "- **Testing Layer:** Testes unitários e de integração com pytest (agent: TestingAgent, skills: [basic_coding]).\n"
            "- **Infrastructure Layer:** Dockerfile, docker-compose.yml (agent: InfrastructureAgent, skills: [basic_coding]).\n"
            "- **Documentation Layer:** README.md (agent: DocumentationAgent, skills: [basic_coding]).\n"
            "\n"
            "- Se o domínio for 'Analytics': O plano DEVE conter Data, Database, Analytics, Testing e Infrastructure.\n"
            "- Se for 'Ecommerce' ou 'CRM': O plano DEVE conter Database, Backend, Frontend, Testing e Infrastructure.\n"
